...
class Window:
    ...
    def __init__(self, on_click_callback=None):
        ...
        self.themes = ["Simple", "Classic", "Sun", "Fire"]
    ...
    def _draw_menu(self):
        ...
        theme_area = pygame.Rect(right.x + 24, theme_title_y + 30, right.width - 48, 60)

        cols = len(self.themes)
        gap = 10
        button_w = (theme_area.width - gap * (cols - 1)) // cols

        for i, theme in enumerate(self.themes):
            rect = pygame.Rect(
                theme_area.x + i * (button_w + gap),
                theme_area.y,
                button_w,
                40,
            )
            self._button(
                rect,
                theme,
                lambda t=theme: self._load_theme(t),
                selected=self.current_theme == theme,
            )

        launch_rect = pygame.Rect(right.x + 24, right.bottom - 60, right.width - 48, 52)
